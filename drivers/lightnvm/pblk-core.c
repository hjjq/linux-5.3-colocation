...
u64 pblk_alloc_next_lun(struct pblk *pblk, struct pblk_line **pline, int nr_secs)
{
    ...
    // Round up to next LUN
    //printk("line->cur_sec before = %u, sec_stripe = %u, rem = %u\n", line->cur_sec, uaddrf->sec_stripe, rem);
    line->cur_sec = line->cur_sec + uaddrf->sec_stripe - rem;
    //printk("line->cur_sec after = %u\n", line->cur_sec);

	while (line->cur_sec + nr_secs > pblk->lm.sec_per_line) {
		//printk("pblk_alloc_next_lun: need to get new line\n");
        struct pblk_line *prev_line = line;
        line = pblk_line_replace_data(pblk);
        if(!line){
            //printk("pblk_alloc_next:lun: no space for new line\n");
            line->cur_sec = line->cur_sec + rem - uaddrf->sec_stripe;
            break;
        }
        pblk_line_close_meta_nolock(pblk,prev_line);
	}

    // cur_sec is either 0 because of new line, or at the next LUN of old line
	line->cur_sec = addr = find_next_zero_bit(line->map_bitmap,
					pblk->lm.sec_per_line, line->cur_sec);
    ...
    //printk("line->cur_sec final = %u, paddr final = %u\n", line->cur_sec, addr);
	line->left_msecs -= (nr_secs + uaddrf->sec_stripe - rem);
    //printk("line->left_msecs = %u, nr_secs = %u\n", line->left_msecs, nr_secs);
	WARN(line->left_msecs < 0, "pblk: page allocation out of bounds\n");
	spin_unlock(&line->lock);
    *pline = line;
	return addr;
