			   ...
			   struct ppa_addr *erase_ppa)
{
    ...
    //printk("pblk_setup_w_rq : nr_secs = %u\n", nr_secs);
	...
	if (likely(!e_line || !atomic_read(&e_line->left_eblks)))
        ...
        //printk("pblk_setup_w_rq: calling erase_rq\n");
...
static int pblk_submit_io_set(struct pblk *pblk, struct nvm_rq *rqd)
{
	...
	if (!pblk_ppa_empty(erase_ppa)) {
        ...
        //printk("pblk_submit_io_set: calling pblk_blk_erase_async\n");
	...
	if (meta_line) {
        //printk("pblk_submit_io_set: calling pblk_submit_meta_io\n");
...
// 2233MOD
/* Function to check if there exists duplicates within the ring buffer
   returns the location in the ring buffer of the other copy
   returns -1 if no duplicate exists
 */
int check_coloc_rb(struct pblk_rb* rb, int pos, int nr_secs){

    int i, j, ptr;
    // Adding below line sometimes causes deadlock
    //pblk_rb_sync_init(rb, NULL);
    u64 coloc_lbas[nr_secs];
    for (i = 0; i < nr_secs; i++) {
       ptr = pblk_rb_ptr_wrap(rb, pos, i);
       coloc_lbas[i] = rb->entries[ptr].w_ctx.lba; 

       // loop through entries in the buffer prior to current entry i, and find copies
       for (j = i - 1; j >= 0; j--) {
           if (rb->entries[ptr].w_ctx.coloc_lba == coloc_lbas[j]) {
               //printk("colocation in rb : copy lba = %u, orig lba = %u\n", coloc_lbas[i], coloc_lbas[j]);
               //Release lock
...
static int pblk_submit_write(struct pblk *pblk, int *secs_left)
{
    ...
    int coloc_found_pos = -1, new_secs_to_sync;
    unsigned int rbio_res;
	...
	*secs_left = 0;
	...
	if (resubmit) {
        ...
        // 2233MOD
        pos = READ_ONCE(pblk->rwb.subm);
        coloc_found_pos = check_coloc_rb(&pblk->rwb, pos, secs_to_com);
        if(coloc_found_pos != -1) { // found duplicate
            // split the vector by changing secs_to_com
            new_secs_to_sync = coloc_found_pos - pos;
            secs_to_com = (new_secs_to_sync > secs_avail) ?
                secs_avail : new_secs_to_sync;
        }
        pos = pblk_rb_read_commit(&pblk->rwb, secs_to_com);

        /* example code to swap entries within the buffer. not used
        struct pblk_w_ctx tmp_wctx = pblk->rwb.entries[pos + 1].w_ctx;
        void* tmp_data = pblk->rwb.entries[pos + 1].data;
        pblk->rwb.entries[pos + 1].w_ctx = pblk->rwb.entries[pos+2].w_ctx;
        pblk->rwb.entries[pos + 1].data = pblk->rwb.entries[pos+2].data;
        pblk->rwb.entries[pos + 2].w_ctx = tmp_wctx; 
        pblk->rwb.entries[pos + 2].data = tmp_data;
        pblk_update_map_cache(pblk, pblk->rwb.entries[pos + 1].w_ctx.lba, pblk->rwb.entries[pos + 1].cacheline);
        pblk_update_map_cache(pblk, pblk->rwb.entries[pos + 2].w_ctx.lba, pblk->rwb.entries[pos + 2].cacheline);
        */
	}
    ...
    // 2233MOD
    if (coloc_found_pos != -1 ){
        // if vector is split, pass in secs_to_com instead of secs_avail
        rbio_res =  pblk_rb_read_to_bio(&pblk->rwb, rqd, pos, secs_to_sync, secs_to_com);
    }else {
        rbio_res =  pblk_rb_read_to_bio(&pblk->rwb, rqd, pos, secs_to_sync, secs_avail);
    }
