...
// LBA of a duplicate pair, including the number of sectors
struct colocation_pair {
    sector_t lba_orig;
    sector_t lba_copy;
    int num_sectors;
};
// used to record an (LBA, LUN) pair in a linux hashmap
struct lba_lun {
    sector_t lba;
    int ch;
    int lun;
...
// MRU FIFOs
extern void* mru_addr[MRU_SIZE];
extern unsigned mru_data[MRU_SIZE];
extern sector_t mru_lba[MRU_SIZE];
extern int mru_newest;
// number of co-location encounters
extern int colocation_count;
// number of duplicate encounters, devide count by check to get probability of co-location
extern int colocation_checks;
extern struct colocation_pair copair[MRU_SIZE];
extern int copair_newest;
extern DECLARE_HASHTABLE(lba_lun_map, HASH_SIZE_BITS);
...
struct pblk_w_ctx {
    ...
    // 2233MOD
    // Add an entry for the other copy's LBA, so that the consumer of the
    // write buffer can check for duplicates
    int coloc_lba;
};
 ...
 */
// 2233MOD
// Function declaration, though these 2 functions were never used..
void pblk_line_close_meta_nolock(struct pblk *pblk, struct pblk_line *line);
u64 pblk_alloc_next_lun(struct pblk *pblk, struct pblk_line **pline, int nr_secs);
